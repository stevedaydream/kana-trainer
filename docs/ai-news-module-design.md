...
- **🗣️ 口說**：借 `shadowScreen`（聽→哼→影子）跑 `speaking` 行；**語速選擇**＝既有 rate；**難度**＝選哪些行。發音評分分階段（見 §6）。

---

## 6. 口說評分（已定：MVP 自評 ＋ 錄音回放）

- **採用：自評 + 錄音回放**。用 `MediaRecorder` 錄下自己朗讀，播**原聲（本機 TTS）**與**自錄**對照，使用者自評。純前端、可離線、零成本、錄音**留在本機不外傳**（隱私）。搭配既有 `shadowScreen`（聽→哼→影子）與語速選擇。
- **不做**自動評分。以下列為**後續（非本設計範圍）**：
...
- **新增狀態**：`P.news = { level:"N5", furigana:true, doneDays:{}, quizBest:{} }`，選配 `P.newsVocab`（單字 SRS）。
